...
def inject_global_data():
    return {
        "exam_target_date": getattr(payment_config, "EXAM_TARGET_DATE", "2026-11-22T08:00:00"),
        "exam_date_label": getattr(payment_config, "EXAM_DATE_LABEL", "22 de Noviembre, 2026"),
        "plans": payment_config.PLANS,
        "bank_accounts": payment_config.BANK_ACCOUNTS
    }

def get_device_id_from_request():
    """Extracts device_id from header, cookie, session or query param."""
    return request.headers.get("X-Device-Id") or request.cookies.get("notariado_device_id") or session.get("device_id") or request.args.get("device_id")
...
def activar():
    ...
    if request.method == "POST":
        ...
        if result["success"]:
            # Store in session
            session["device_id"] = client_device_id
            session["is_licensed"] = True
            session["license_key"] = key
            # Set cookie for persistence
            resp = make_response(jsonify(result))
            resp.set_cookie("notariado_device_id", client_device_id, max_age=365*24*3600, httponly=False, samesite="Lax")
            return resp
...
@app.route("/resultado/<attempt_id>")
